#!/usr/bin/env python3
"""
Session 154 smoke: wmedit Ctrl-Y redo (after Ctrl-Z undo).

Type "hello"; baseline screendump has text.
Ctrl-Z: text gone.
Ctrl-Y: text back.

Pixel checks:
  - baseline: green text in body
  - after Ctrl-Z: no green text
  - after Ctrl-Y: green text restored (count matches baseline)
"""
...
def main():
    ...
    log = open(os.path.join(ROOT, "qemu-s154.log"), "w")
    ...
    try:
        ...
        time.sleep(2.0)
        ser.sendall(b"wmedit /tmp/redo 50\n")
        time.sleep(4.0)
        ...
        def shot(name):
            path = os.path.join(ROOT, f"shot_redo_{name}.ppm")
        ...
        print(f"   baseline: {green_a} green px")
        ...
        time.sleep(1.2)
        _, _, pxC = shot("c")
        green_c = count_green(pxC)
        print(f"   after Ctrl-Y: {green_c} green px")

        # wmd top status bar.
        sb = sum(1 for xx in range(50, w - 50)
                 if near(px_at(pxC, xx, 6), (0x20, 0x20, 0x20), tol=10))

        checks = []
        checks.append((f"baseline text rendered ({green_a} px)",
                       green_a > 50))
        checks.append((f"undo cleared text ({green_b} px remaining)",
                       green_b < green_a / 4))
        checks.append((f"redo restored text ({green_c} px, baseline was {green_a})",
                       green_c > green_a * 3 // 4))
