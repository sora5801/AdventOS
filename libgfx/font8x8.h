#ifndef ADVENTOS_FONT8X8_H
#define ADVENTOS_FONT8X8_H

#include "../include/types.h"

/*
 * 8x8 monospace bitmap font for ASCII 0x20..0x7F.
 *
 * Adapted from the well-known public-domain "font8x8_basic" data
 * (Marcel Sondaar / dhepper, originally by IBM for the PC BIOS
 * character ROM — released to the public domain). Each glyph is
 * 8 bytes; row n covers pixels at y = baseline + n. Bit 0 of each
 * byte is the leftmost pixel; bit 7 is the rightmost.
 *
 * To render glyph c at (x,y) on a 32-bpp framebuffer:
 *   for (int row = 0; row < 8; row++) {
 *       uint8_t bits = font8x8[c - 0x20][row];
 *       for (int col = 0; col < 8; col++)
 *           if (bits & (1 << col))
 *               fb[(y+row)*pitch + (x+col)] = fg;
 *   }
 */

#define FONT_FIRST_CH  0x20
#define FONT_LAST_CH   0x7F
#define FONT_W         8
#define FONT_H         8

static const uint8_t font8x8[FONT_LAST_CH - FONT_FIRST_CH + 1][FONT_H] = {
