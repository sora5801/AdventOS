...
echo "OK. Run with (recommended — graphical QEMU window + USB keyboard):"
...
echo ""
echo "  Type into the QEMU graphical window — keys land in the kernel's"
echo "  kbd ring via the USB-HID polling task (usb_hid_kbd_task in"
echo "  kernel/usb_hid.c). Output also tees to the host terminal because"
echo "  of -serial stdio."
echo ""
echo "  -smp 1 stays the recommended SMP setting; -smp 2 + agentd in"
echo "  inittab hangs in TCP loopback under load. See"
echo "  docs/66-smp-loopback-fix.md."
echo ""
...
echo "Session 68 added a PIT-tick polling fallback in kernel/serial.c so"
echo "ANY bytes that do reach the UART RBR get drained, but in practice"
echo "they don't arrive. If you want headless, drive the OS through the"
echo "agentd JSON-RPC endpoint on 127.0.0.1:7000 (curl-friendly) or"
echo "ssh into it on port 2222 — both still work under -display none."
echo ""
