...
echo "Note: -smp 1 stays the recommended config — session 66 closed one"
echo "      class of SMP races (added g_net_lock for the rtl8139 IRQ vs"
echo "      sock layer hazard) but the original t20 nc-localhost hang"
echo "      under -smp 2 with agentd in inittab persists, suggesting a"
echo "      separate scheduler or BKL contention issue. See"
echo "      docs/66-smp-loopback-fix.md for the diagnosis."
echo ""
echo "Headless input: -display none + -serial stdio is the agent-target"
echo "      boot mode. Session 67 wired the COM1 RX IRQ into the kbd ring,"
echo "      so typing at the host terminal (or piping commands into QEMU's"
echo "      stdin) reaches the shell prompt directly — no PS/2 / USB-kbd"
echo "      device needed. \\r becomes \\n and 0x7F becomes \\b at the"
echo "      driver boundary; Ctrl-C (0x03) passes through. See"
echo "      docs/67-serial-keyboard-input.md."
echo ""
