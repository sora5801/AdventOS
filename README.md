...
| Native compiler — `cc` (C-subset: int, char, pointers, arrays, strings, globals, `printf`, preprocessor, compound ops, structs (incl. arrays-of), function pointers, `sizeof`, scaled ptr arith, multi-file, struct value assign, enum; emits ELF32) | ✅ |
...
Current session: **103 — cc Phase 3 part 7: enums**. See [`docs/90-cc-enums.md`](docs/90-cc-enums.md) for the deep dive (T_ENUM keyword, top-level enum parser, g_enum_consts table, parse_primary substitutes N_NUM at parse time). Named constants for flags, error codes, state machines.
