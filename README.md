...
| Native compiler — `cc` (C-subset: int, char, pointers, arrays, strings, globals, `printf`, preprocessor, compound ops, structs (incl. arrays-of), function pointers, `sizeof`, scaled ptr arith, multi-file, struct value assign, enum, typedef; emits ELF32) | ✅ |
...
Current session: **105 — cc Phase 3 part 9: variadic functions**. See [`docs/92-cc-variadics.md`](docs/92-cc-variadics.md) for the deep dive (T_ELLIPSIS in param lists, `is_variadic` flag relaxing argc check, three new intrinsics `va_start`/`va_arg`/`va_end` that walk the i386 cdecl stack frame).
