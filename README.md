...
Current session: **120 — Path B Phase 4 capstone**. See [`docs/108-pathB-capstone.md`](docs/108-pathB-capstone.md) for the deep dive. Closes out the C-subset language surface in `cc`: struct-by-value returns (hidden-first-arg cdecl ABI), `static` / `extern` storage-class keywords, and the `typedef RET (*NAME)(ARGS);` function-pointer typedef syntax. Smoke test ([`smoke_pathB.py`](smoke_pathB.py) + [`fs/capstone.c`](fs/capstone.c)) compiles a program exercising all three features and verifies eight expected output lines including the SBV-return-doesn't-alias invariant.
...
- **Path B optimization** — register allocator, constant folding, peephole. cc's output is correct but bulky. Or alternately a real `tcc` port for full-C support.
