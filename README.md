...
| Native compiler — `cc` (C-subset: int, char, pointers, arrays, string literals, emits ELF32) | ✅ |
...
Current session: **92 — cc Phase 2 part 2: char + pointers + arrays + & / \***. See [`docs/79-cc-chars-pointers.md`](docs/79-cc-chars-pointers.md) for the deep dive (LK_* type tags on locals/params, name-decay-to-address for arrays, byte vs dword load/store, scaled indexing). `my_strlen` and `my_strcpy` written in the cc-subset and run end-to-end.
...
**Path B — Self-hosting is in progress.** Session 90 (Phase 1) shipped a 1500-line C-subset compiler — int-only, no strings/pointers/globals/preprocessor; see [`docs/77-tinycc.md`](docs/77-tinycc.md). Session 91 (Phase 2 part 1) added string literals + `puts`/`print_str` so `puts("hello, world")` works; see [`docs/78-cc-strings.md`](docs/78-cc-strings.md). Session 92 (Phase 2 part 2) added char + pointers + arrays + `&` / `*` — `my_strlen` / `my_strcpy` now compile and run; see [`docs/79-cc-chars-pointers.md`](docs/79-cc-chars-pointers.md). Remaining Phase 2 sub-sessions: globals (93), `printf` (94), preprocessor (95), compound ops (96).
