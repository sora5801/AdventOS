...
| Native compiler — `cc` (C-subset: int, char, pointers, arrays, strings, globals, `printf`, preprocessor, compound ops, structs, function pointers, `sizeof`, scaled ptr arith, multi-file; emits ELF32) | ✅ |
...
Current session: **101 — cc Phase 3 part 5: struct value assignment**. See [`docs/88-cc-struct-value-assign.md`](docs/88-cc-struct-value-assign.md) for the deep dive (LHS struct kind branches N_ASSIGN into a `rep movsd` memcpy; push/pop esi/edi for the callee-saved registers). `copy = orig;` is now a single statement.
