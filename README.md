...
| Native compiler — `cc` (C-subset: int, char, pointers, arrays, strings, globals, `printf`, preprocessor, compound ops; emits ELF32) | ✅ |
...
Current session: **96 — cc Phase 2 complete: compound operators**. See [`docs/83-cc-compound-ops.md`](docs/83-cc-compound-ops.md) for the deep dive (`+=`/`-=`/etc. via parser rewrite, `++`/`--` prefix and postfix, ternary `?:`). **Path B Phase 2 is now complete.** cc is ~2100 lines across one file and compiles programs that exercise int + char + pointers + arrays + globals + printf + preprocessor + the natural operators.
...
**Path B — Self-hosting Phase 2 is complete.** Session 90 (Phase 1) shipped a 1500-line C-subset compiler — int-only; see [`docs/77-tinycc.md`](docs/77-tinycc.md). Sessions 91–96 added string literals + `puts`/`print_str`, char + pointers + arrays + `&` / `*`, global variables, `printf` (compile-time-dispatched intrinsic with `%d`/`%s`/`%c`/`%x`/`%%`), the preprocessor (`#define` / `#undef` / `#include` / `#ifdef` / `#ifndef` / `#else` / `#endif` with classic header-guard support), and compound operators (`+=` / `-=` / `*=` / `/=` / `%=` / `++` / `--` / ternary `?:`). cc is ~2100 lines now and compiles real-shaped C programs end-to-end.
