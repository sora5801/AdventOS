...
| Native compiler — `cc` (C-subset: int, char, pointers, arrays, strings, globals, `printf`, preprocessor, compound ops, structs (incl. arrays-of), function pointers, `sizeof`, scaled ptr arith, multi-file, struct value assign; emits ELF32) | ✅ |
...
Current session: **102 — cc Phase 3 part 6: array-of-struct + indexed member access**. See [`docs/89-cc-array-of-struct.md`](docs/89-cc-array-of-struct.md) for the deep dive (`LK_STRUCT_ARR` kind, `pts[i].x = ...` and `items[i].name` parser/codegen). The canonical "table of records" pattern works now.
